...
export default function AuthPage({ initialTab = "signup", onBack, onSuccess }) {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.google?.accounts?.id && !window.__googleInitialized) {
        try {
    ...
    return () => {
      // Don't remove the script - it causes re-initialization issues
    };
  }, []);
  ...
  const handleGoogleCallback = async (response) => {
    ...
    try {
      ...
      if (data.profile_completed) {
        // Profile done, proceed to workspace
      ...
      <div style={{
        ...
        {isSubmitting ? (
          ...
          <div>
            ...
            {/* Google Sign-In Button - Simple Method */}
            <button 
              type="button"
              onClick={() => {
                // Open Google OAuth directly
                const clientId = "535569661906-grbfrkr877hc0hgtsu1sc23pic84dhlt.apps.googleusercontent.com";
              ...
              style={{
                ...
                border: "1.5px solid rgba(148,41,69,0.2)", background: "#fff",
              ...
              onMouseEnter={e => {
                e.target.style.borderColor = "#942945";
                e.target.style.background = "rgba(148,41,69,0.02)";
              }}
              onMouseLeave={e => {
                e.target.style.borderColor = "rgba(148,41,69,0.2)";
                e.target.style.background = "#fff";
              }}
            >
